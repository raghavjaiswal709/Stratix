...
export function ExecutionPanel({
  ...
  return (
    <div className="w-full h-full flex flex-col bg-transparent text-white/65 select-none font-sans">
      {/* Symbol Title + Tabs */}
      <div className="flex flex-col shrink-0 border-b border-white/[0.08] bg-transparent">
        ...
        <div className="flex px-2 pb-2 gap-1">
          ...
          <button
            ...
            className={`flex-1 py-1.5 text-[10px] font-bold tracking-wide rounded-md transition-all flex items-center justify-center gap-1 ${
              activeTab === "trades"
                ? "bg-white/[0.10] text-white"
                : "text-white/35 hover:text-white hover:bg-white/[0.05]"
            }`}
          >
            ...
            Trades {sessionTrades.length > 0 && <span className="ml-0.5 px-1 rounded-full bg-white/[0.12] text-[8px]">{sessionTrades.length}</span>}
          </button>
        </div>
      </div>
      ...
      {/* ── Trades Tab ── */}
      {activeTab === "trades" && (
        <div className="flex-1 overflow-y-auto flex flex-col font-mono">
          {sessionTrades.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-2 text-white/20 p-6">
              <ListOrdered className="w-8 h-8 opacity-30" />
              <span className="text-[10px] uppercase tracking-wider font-bold">No trades yet</span>
              <span className="text-[9px] text-white/15 text-center">Trades placed during replay will appear here</span>
            </div>
          ) : (
            <div className="p-2 flex flex-col gap-1.5">
              {[...sessionTrades].reverse().map((t, i) => {
                const win = (t.pnl ?? 0) >= 0;
                return (
                  <div key={t.id} className={`rounded-lg border p-2.5 flex flex-col gap-1.5 ${
                    win ? "border-emerald-500/[0.15] bg-emerald-500/[0.04]" : "border-red-500/[0.15] bg-red-500/[0.04]"
                  }`}>
                    {/* Header row */}
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[8px] text-white/25">{t.lotSize} lot</span>
                      </div>
                      <span className={`text-[10px] font-bold ${win ? "text-emerald-400" : "text-red-400"}`}>
                        {win ? "+" : ""}${(t.pnl ?? 0).toFixed(2)}
                      </span>
                    </div>
                    {/* Price row */}
                    <div className="flex items-center justify-between text-[8px] text-white/35">
                      <span>Entry <b className="text-white/65">{fmtP(t.entryPrice)}</b></span>
                      {t.exitPrice != null && (
                        <span>Exit <b className="text-white/65">{fmtP(t.exitPrice)}</b></span>
                      )}
                    </div>
                    {/* Time row */}
                    <div className="flex items-center justify-between text-[7.5px] text-white/20">
                      <span>{fmtTime(t.entryTime)}</span>
                      {t.exitTime != null && <span>→ {fmtTime(t.exitTime)}</span>}
                    </div>
                  </div>
                );
              })}
