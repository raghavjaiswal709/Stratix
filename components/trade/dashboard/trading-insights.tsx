...
import { deriveInsights, type DashboardStats } from "@/lib/trades/dashboard-stats";
...
export function TradingInsights({ stats }: Props) {
  // Every figure below is rolled up in Mongo now (lib/trades/stats-pipeline.ts);
  // deriveInsights only does the final ratios, the symbol sort, and the
  // order-dependent streak walk. Formulas and field names are unchanged.
  const m = useMemo(() => deriveInsights(stats), [stats]);
