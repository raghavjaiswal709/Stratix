...
import { X, Sparkles, Loader2, Calendar, CalendarDays, CalendarRange, Infinity as InfinityIcon } from "lucide-react";
import { cn } from "@/lib/utils";

export type ReportTimeRange = "week" | "month" | "3months" | "all";

const OPTIONS: { value: ReportTimeRange; label: string; sub: string; icon: typeof Calendar }[] = [
  { value: "week", label: "Last 7 Days", sub: "Quick recent check-in", icon: Calendar },
  { value: "month", label: "Last 30 Days", sub: "Most common window", icon: CalendarDays },
  { value: "3months", label: "Last 3 Months", sub: "Broader pattern view", icon: CalendarRange },
  { value: "all", label: "All Time", sub: "Full trading history", icon: InfinityIcon },
];
...
export function AnalyzeModal({
  ...
  onClose: () => void;
  onGenerate: (timeRange: ReportTimeRange) => void;
  generating: boolean;
  error: string | null;
}) {
  const [selected, setSelected] = useState<ReportTimeRange>("month");

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className={cn(
        "w-full max-w-md rounded-2xl bg-[#0c0e14] border border-white/10 shadow-2xl overflow-hidden relative transition-all",
        ...
        <div className="flex items-center gap-3 p-5 border-b border-white/7">
          ...
          <div className="flex-1">
            <h3 className="text-[15px] font-semibold text-white">Generate AI Report</h3>
            <p className="text-[11px] text-white/40 mt-0.5">
              {generating ? "Reading trades, journal notes & missed setups…" : "Choose the time period to analyze"}
            </p>
          </div>
        ...
        <div className={cn("p-5 space-y-2 transition", generating && "opacity-40 pointer-events-none")}>
          {OPTIONS.map(opt => {
            const Icon = opt.icon;
            return (
              <button
                ...
                disabled={generating}
                className={cn(
                  "w-full flex items-center gap-3 p-3 rounded-xl border text-left transition disabled:opacity-50",
                  selected === opt.value
              ...
              >
                ...
                <div className="flex-1">
                  <p className={cn("text-[13px] font-semibold", selected === opt.value ? "text-white" : "text-white/70")}>{opt.label}</p>
          ...
          {error && (
            <div className="rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-2.5 text-[11px] text-red-300">
              {error}
            </div>
          )}
        </div>
