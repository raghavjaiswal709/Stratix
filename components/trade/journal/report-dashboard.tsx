...
export interface ReportData {
  ...
  choch_qml_analysis: {
    score: number;
    summary: string;
    confirmation_compliance_rate: number;
    strengths: string[];
    weaknesses: string[];
  };
      ...
      <div className="px-6 py-5 space-y-5 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="rounded-xl border border-amber-500/15 bg-amber-500/[0.02] overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-white/7">
              <Target className="h-4 w-4 text-amber-400/80" />
              <span className="text-[12px] font-semibold text-white/70 uppercase tracking-wider">CHOCH / QML Score</span>
              <span className="ml-auto text-[10px] text-white/35">{d.choch_qml_analysis.confirmation_compliance_rate}% confirmed</span>
            </div>
            <div className="p-4 flex items-start gap-4">
              <ScoreRing score={d.choch_qml_analysis.score} />
              <p className="text-[12px] text-white/65 leading-relaxed flex-1 line-clamp-5">{d.choch_qml_analysis.summary}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-white/7 overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-white/7">
            <Target className="h-4 w-4 text-amber-400/80" />
            <span className="text-[12px] font-semibold text-white/70 uppercase tracking-wider">CHOCH / QML Deep Dive</span>
          </div>
          <div className="p-4 space-y-4">
            <p className="text-[13px] text-white/70 leading-relaxed whitespace-pre-wrap">{d.choch_qml_analysis.summary}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-emerald-400/70 font-semibold mb-2">Strengths</p>
                <ul className="space-y-1.5">
                  {d.choch_qml_analysis.strengths.map((s, i) => (
                    <li key={i} className="text-[12px] text-white/65 flex gap-2">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-wider text-red-400/70 font-semibold mb-2">Weaknesses</p>
                <ul className="space-y-1.5">
                  {d.choch_qml_analysis.weaknesses.map((s, i) => (
                    <li key={i} className="text-[12px] text-white/65 flex gap-2">
        ...
        {/* Execution checklist compliance */}
        <div className="rounded-xl border border-white/7 overflow-hidden">
